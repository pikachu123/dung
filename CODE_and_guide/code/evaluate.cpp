...
double evaluate_3(vector<int> s1, vector<int> s2, vector<int> s3, int numchannel){
	double a12 = evaluate_2(s1,s2,numchannel);//cout<<"a12 = "<<a12<<endl;
	double a13 = evaluate_2(s1,s3,numchannel);//cout<<"a13 = "<<a13<<endl;
	double a23 = evaluate_2(s2,s3,numchannel);//cout<<"a23 = "<<a23<<endl;
	return (a12+a13+a23)/3;
}

double evaluate_2(vector<int> s1, vector<int> s2, int numchannel){ //numchannel is # of channel; value = 2 means that channel = 0, 1;
	int size_1 = s1.size();
	int size_2 = s2.size();
	int sum = 0;
	double result=0;
...
int subevaluate(vector<int> s1, vector<int> s2, int numchannel, int start1, int start2){
